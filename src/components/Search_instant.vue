        ...
        <div class="row">
                    ...
                    <div class="form-control dropdown mt-1 ht-45  flex-column" style="padding: 0%;" @click="handleDropdownClick"> 
                        ...
                        <div v-if="filteredItems.length" class="dropdown-list">
                            <div v-for="(item, index) in filteredItems" :key="index" @click="selectItem(item)" :class="{'selected': index === selectedIndex}" class="dropdown-item">
                                <span class="push-button" @click.stop="togglePush(item)" title="Include/Exclude&#10;包含/排除 此特征&#10;&#10;点击复选框切换">{{ item.include ? '＋' : '－' }}</span>
            ...
            <div class="col-8 ">
                <div class="input-group mb-0 mt-0 ">
                    ...
                    <input class="form-control dark-mode  mt-1 ht-45" v-model="searchPattern" placeholder="全文搜索关键字：搜索模式 或 关键字 或 正则表达式"
...
export default {
    ...
    setup() {
        ...
        const items= ref([
            ...
            { include:true,type:'doc',content: '*.docx *.doc *.odt *.ods *.odf *.rtf' },
            ...
            { include:true,type:'js',content: '*.cjs *.js *.jsx *.mjs *.vue' },
            ...
            { include:true,type:'md',content: '*.markdown *.md *.mdown *.mdwn *.mdx *.mkd *.mkdn' },
            ...
            { include:true,type:'pic',content: '*.[jJ][pP][gG] *.[jJ][pP][eE][gG] *.[bB][mM][pP] *.[pP][nN][gG] *.[gG][iI][fF] *.tiff *.raw *.svg *.psd *.eps' },
            ...
            { include:true,type:'py',content: '*.py *.pyi' },
            ...
            { include:true,type:'ts',content: '*.cts *.mts *.ts *.tsx' },
        ...
        const handleKeydown=(event)=> {
            if (event.key === 'ArrowDown') {
                ...
                if (selectedIndex.value >= 0) {
                    filenamePattern.value = filteredItems.value[selectedIndex.value].content;
                    filteredItems.value = [];
                }
            }
        };
